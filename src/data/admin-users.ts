...
import type { AccountStatus, AdminUserProfileInput } from "@/lib/admin-user-validation"
...
export type AuditValues = Partial<Record<
  "Email" | "DisplayName" | "FName" | "Mname" | "LName" | "MobileNum" | "account_status" | "role",
  string | null
>>
...
export async function getAdminDashboardData() {
  ...
  return {
    ...
    auditLogs: { data: (auditLogs.data ?? []) as PubUserAuditRow[], error: auditLogs.error },
    resetLogs: { data: (resetLogs.data ?? []) as PasswordResetLogRow[], error: resetLogs.error },
  }
}
...
export async function requestAdminApi<TBody extends Record<string, unknown> = Record<string, unknown>>(
  path: string,
  init: RequestInit = {},
) {
...
export function createAdminUserAccount(input: AdminUserProfileInput) {
  return requestAdminApi<{ message: string; user: AdminUserRow }>("/api/admin/users", {
    method: "POST",
    body: JSON.stringify(input),
  })
}

export function updateAdminUserAccount(userId: string, input: AdminUserProfileInput) {
  return requestAdminApi<{ message: string; user: AdminUserRow }>(`/api/admin/users/${encodeURIComponent(userId)}`, {
    method: "PATCH",
    body: JSON.stringify(input),
  })
}

export function setAdminUserAccountStatus(userId: string, accountStatus: AccountStatus) {
  return requestAdminApi<{ message: string; user: AdminUserRow }>(`/api/admin/users/${encodeURIComponent(userId)}`, {
    method: "PUT",
