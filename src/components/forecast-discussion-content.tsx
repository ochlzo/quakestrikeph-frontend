...
function likelihoodStyle(level: string | null) {
  switch (level?.toLowerCase()) {
    case "high": return "bg-likelihood-high/10 text-likelihood-high"
    case "medium": return "bg-likelihood-medium/10 text-likelihood-medium"
    case "low": return "bg-likelihood-low/10 text-likelihood-low"
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-xl border bg-card p-6 text-center shadow-sm">
        ...
        {forecast.aftershock24hLikelihoodLevel ? (
          <span className={`mt-3 inline-flex rounded-full px-2.5 py-1 text-xs font-semibold uppercase ${likelihoodStyle(forecast.aftershock24hLikelihoodLevel)}`}>
            {forecast.aftershock24hLikelihoodLevel}
          </span>
        ) : null}
      </section>
...
export function ForecastDiscussionSections({
  ...
  const nearest = firstDay.reduce<ForecastPlaybackEvent | null>(
    (current, event) => !current || event.distanceKm < current.distanceKm ? event : current,
    null
  )
  const magnitudeFiveCount = firstDay.filter((event) => event.magnitude >= 5).length

  return (
    <div className="mx-auto max-w-5xl space-y-10 py-12">
      <section className="space-y-3">
        <h2 className="text-2xl font-semibold tracking-tight">What this forecast says</h2>
        <p className="leading-7 text-muted-foreground">
          This is a probability estimate, not a prediction of the exact time or location of an earthquake. It describes what the model considers plausible during the fixed 24-hour period beginning when the forecast was generated.
        </p>
        <p className="leading-7 text-muted-foreground">
          The chance of a magnitude 5 or stronger aftershock is {probability(forecast.m5PlusAftershock)}. The model estimates the strongest aftershock at {forecast.estimatedStrongestAftershock === null ? "an unavailable magnitude" : `about M${forecast.estimatedStrongestAftershock.toFixed(1)}`}, with {getMostLikelyDistance(forecast).toLowerCase()} as the most-likely distance from the trigger.
        </p>
      </section>
      ...
      <section className="space-y-4">
        ...
        {strongest ? (
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <div className="rounded-lg bg-muted p-4">
              <p className="text-xs uppercase tracking-wide text-muted-foreground">Largest observed</p>
              <strong className="mt-1 block text-2xl">M{strongest.magnitude.toFixed(1)}</strong>
            </div>
            <div className="rounded-lg bg-muted p-4">
              <p className="text-xs uppercase tracking-wide text-muted-foreground">Nearest observed</p>
              <strong className="mt-1 block text-2xl">{nearest?.distanceKm.toFixed(1)} km</strong>
            </div>
          </div>
        ) : null}
      ...
      <section className="grid gap-8 md:grid-cols-2">
        <div className="space-y-3">
          <h2 className="text-2xl font-semibold tracking-tight">Where activity was considered likely</h2>
          <p className="leading-7 text-muted-foreground">
            These bands show the model’s probability distribution for distance from the trigger. They do not define confirmed aftershock boundaries.
          </p>
          {forecast.distanceMessage ? (
            <p className="text-sm leading-6 text-muted-foreground">{forecast.distanceMessage}</p>
          ) : null}
        </div>
        <dl className="divide-y rounded-xl border bg-card px-4">
          {FORECAST_DISTANCE_BANDS.map((band) => (
            <div key={band.key} className="flex items-center justify-between gap-4 py-3">
              <dt>{band.label}</dt>
              <dd className="font-semibold tabular-nums">{probability(forecast[band.key])}</dd>
            </div>
          ))}
        </dl>
      </section>
      ...
      <section className="space-y-4">
        <h2 className="text-2xl font-semibold tracking-tight">Forecast and source details</h2>
        <dl className="grid gap-x-8 gap-y-4 rounded-xl border bg-muted/50 p-5 sm:grid-cols-2">
          <div><dt className="text-xs uppercase tracking-wide text-muted-foreground">Trigger event</dt><dd className="mt-1 font-medium">{trigger.id}</dd></div>
